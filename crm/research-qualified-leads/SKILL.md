...
4. Verify the canonical company website, geography, current qualifying signal, job title where applicable, and strongest source. For every job-derived lead on every platform, open the individual vacancy, verify that it shows one vacancy with the matching employer and title, and save its final canonical job-detail URL as `primary_source_url`; never save a search result, keyword/category URL, generic jobs page, or snippet URL. Do not use domain-specific exceptions or URL-shape assumptions. If the actual offer cannot be opened and verified, reject the candidate and replace it. Reject Werkstudent, internship, minijob, apprenticeship, student-assistant, volunteer, and equivalent low-compensation roles. Read [research-policy.md](references/research-policy.md) for source, evidence, freshness, job-detail URL, budget, contact, and exclusion rules.
...
6. For every otherwise-qualifying candidate, perform the mandatory budget-enrichment pass in [research-policy.md](references/research-policy.md). Check the job-detail and official ATS pages first, then job-specific estimates and current title/geography/employment-matched salary or rate benchmarks. Record `budget_enrichment.attempted = true` and every checked source. Label estimates as `ESTIMATED`, record their basis, and record the offer type. Only leave `budget` empty after both published and sufficiently matched estimated values are unavailable.
...
For job-derived candidates in either mode, require the published job title and the opened individual job-offer URL. Require that URL in both the job evidence and `primary_source_url` so `Referrer / Link URL` always contains the actual offer. Reject search/category/landing URLs as `JOB_DETAIL_URL_REQUIRED`. Reject the candidate as `LOW_COMPENSATION_ROLE` when every qualifying job signal is a Werkstudent/working-student role, internship/Praktikum, minijob, apprenticeship/Ausbildung, student-assistant role, volunteer role, or an equivalent explicitly student/training engagement. Do not reject an ordinary junior or trainee role solely because it may pay less.
...
- Keep unknown values empty. Never fabricate evidence, contacts, dates, budget, intent, or outsourcing willingness. A sourced estimate is allowed only when explicitly marked `ESTIMATED` with a documented matching basis.
- Always include a `Budget` CSV column. Require a documented budget-enrichment attempt for every exported candidate. Prefer published compensation, then use a sufficiently matched sourced estimate. Export estimates with the `Geschätzt:` prefix and append the offer type to every populated Budget value.
