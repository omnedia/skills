...
Stable ID: `active-word-highlight`. Use the component in `assets/caption-code/src/CaptionOverlay.tsx`; its pure layout/timeline functions are in `src/layout.mjs`. The shipped `preview.gif` is generated by `scripts/render-check.mjs` from that component's actual alpha MOV, with a dark background added only for the gallery.
