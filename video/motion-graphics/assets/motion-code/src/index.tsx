...
import {sceneState, layerState, strokeState} from './state.mjs';

const Layers=({scene,project,frame}:any)=><>{scene.layers.map((l:any)=>{
  const t=layerState(scene,l,frame),r=l.rect;
  return <div key={l.id} style={{position:'absolute',left:r.x,top:r.y,width:r.width,height:r.height,opacity:t.opacity,filter:l.grayscale?`grayscale(${l.grayscale})`:undefined,transformOrigin:`${l.pivot[0]*100}% ${l.pivot[1]*100}%`,transform:`translate(${t.x}px,${t.y}px) scale(${t.scale})`,maskImage:l.mask?`url(${staticFile(project.assets[l.mask].path)})`:undefined,maskSize:'100% 100%'}}>
...
const renderers:any={editorial:Layers,cutout:Layers,parallax:Layers,paper:Paper,callout:Layers,diagram:Diagram};
function Scene({scene,project,frame}:any){
  ...
  <div style={{position:'absolute',left:p.x,top:p.y,width:p.width,height:p.height,opacity:st.opacity,transform:`translateY(${st.y}px) rotate(${scene.renderer==='paper'?scene.paperTreatment.documentTilt:0}deg)`}}>
    ...
    {(scene.strokes??[]).map((stroke:any,i:number)=>{
      const st=strokeState(scene,stroke,frame),layer=scene.layers.find((l:any)=>l.id===stroke.layerId),t=layer?layerState(scene,layer,frame):null;
      const cx=layer?layer.rect.x+layer.rect.width*layer.pivot[0]:0,cy=layer?layer.rect.y+layer.rect.height*layer.pivot[1]:0;
      return <g key={`stroke-${i}`} transform={t?`translate(${cx+t.x} ${cy+t.y}) scale(${t.scale}) translate(${-cx} ${-cy})`:undefined} opacity={st.progress>0?(t?.opacity??1):0}><path d={st.path} fill="none" stroke={stroke.color??colors.accent} strokeWidth={stroke.width} strokeLinecap="round" strokeLinejoin="round" pathLength={1} strokeDasharray="1 1" strokeDashoffset={1-st.progress}/></g>;
    })}
    {(scene.annotations??[]).map((a:any,i:number)=>{
    ...
    {scene.labels.filter((l:any)=>!l.eventId||frame>=scene.frames.events.find((e:any)=>e.id===l.eventId).frame).map((l:any,i:number)=><div key={i} style={{position:'absolute',left:l.x,top:l.y,width:l.width,height:l.height,fontFamily:project.font.family,fontWeight:700,fontSize:l.fontSize,lineHeight:`${l.lineHeight}px`,color:l.color??colors.ink,whiteSpace:'pre'}}>{l.lines.join('\n')}</div>)}
  </div></>;
}
